      ...
      // {
        ...
        extraInstallerModules = extraInstallerModules ++ [
          ...
          (
            { config, ... }:
            {
              # Modeled on nixpkgs' installer/netboot/netboot.nix, minus the
              # squashfs store bundling (this installer fetches its target
              # system live instead, see snowboot).
      ...
      allModules = [
        ...
        "${sources.agenix}/modules/age.nix"
        {
          ...
          _module.args.operators = mapAttrs' (
            fileName: cfg:
            nameValuePair (cfg.securix.self.username or cfg.securix.self.user.username) (
